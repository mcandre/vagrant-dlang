...
BOX_BASENAME_DEBIAN_I386 = "#{BOX_PREFIX}-debian-i386"
BOX_DEBIAN_I386 = "#{BOX_BASENAME_DEBIAN_I386}.box"
SHORT_DESCRIPTION_DEBIAN_I386 = 'a GNU/Linux (Debian) x86 Vagrant box with D compiler + dub'
...
task :box_debian_i386 => [
    "debian-i386#{File::SEPARATOR}Vagrantfile",
    "debian-i386#{File::SEPARATOR}bootstrap.sh",
    "debian-i386#{File::SEPARATOR}export.Vagrantfile",
    :clean_box_debian_i386
] do
    sh 'vagrant up',
        :chdir => 'debian-i386'
    sh "vagrant package --output #{BOX_DEBIAN_I386} --vagrantfile export.Vagrantfile",
        :chdir => 'debian-i386'
end
...
task :import_debian_i386 => [] do
    sh "vagrant box add --force --name #{BOX_NAMESPACE}/#{BOX_BASENAME_DEBIAN_I386} #{BOX_DEBIAN_I386}",
        :chdir => 'debian-i386'
end
...
task :test_debian_i386 => [
    "debian-i386#{File::SEPARATOR}test#{File::SEPARATOR}Vagrantfile",
    "debian-i386#{File::SEPARATOR}test#{File::SEPARATOR}hello.d"
] do
    sh 'vagrant up',
        :chdir => "debian-i386#{File::SEPARATOR}test"
    sh 'vagrant ssh -c "cd /vagrant && dmd -L -no-pie hello.d && ./hello && dub fetch dscanner && dub run dscanner -- --styleCheck"',
        :chdir => "debian-i386#{File::SEPARATOR}test"
    sh 'vagrant halt',
        :chdir => "debian-i386#{File::SEPARATOR}test"
end
...
task :publish_debian_i386 => [] do
    sh "vagrant cloud publish #{BOX_NAMESPACE}/#{BOX_BASENAME_DEBIAN_I386} --force --release --short-description \"#{SHORT_DESCRIPTION_DEBIAN_I386}\" --version-description \"#{VERSION_DESCRIPTION}\" #{VERSION} #{PROVIDER} #{BOX_DEBIAN_AMD64}",
        :chdir => 'debian-i386'
end
...
task :clean_box_debian_i386 => [] do
    Dir.glob("debian-i386#{File::SEPARATOR}*.box").each { |path| File.delete path }
end

task :clean_debian_i386 => [:clean_box_debian_i386] do
    begin
        sh 'vagrant destroy -f',
            :chdir => 'debian-i386'
    rescue
    ...
    begin
        Dir.glob("debian-i386#{File::SEPARATOR}**#{File::SEPARATOR}.vagrant").each { |path| FileUtils.rm_r path }
    rescue
    end
end
